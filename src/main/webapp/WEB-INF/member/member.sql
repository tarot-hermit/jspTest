...
SELECT mid, nickName, level FROM member WHERE level = 1;

SELECT nickName, level FROM member WHERE nickName = '홍유유';
-- 확인 후 삭제
DELETE FROM member WHERE mid = 'admin';

SELECT nickName, level FROM member WHERE nickName = '김장군';

SELECT COUNT(DISTINCT DATE(visitDate)) FROM guest WHERE name = '김장군';

SELECT g.name, m.nickName, m.level 
FROM guest g 
JOIN member m ON g.name = m.nickName 
WHERE g.name = '김장군';

SELECT mid, pwd, LENGTH(pwd) FROM member WHERE mid = 'admin';

ALTER TABLE member ADD COLUMN deleteDate DATETIME;
